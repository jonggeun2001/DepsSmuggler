...
이 동작은 `npx vitest run src/core/resolver/maven-resolver.test.ts src/renderer/stores/cart-store.test.ts src/renderer/pages/cart-page/maven-pom-parser.test.ts electron/services/download-package-router.test.ts`로 검증합니다.
