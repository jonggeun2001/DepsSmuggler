...
- **버전 목록 조회 실패**: 선택 화면에 경고와 재시도 버튼을 표시합니다. 검색 결과에 이미 포함된 버전은 계속 선택할 수 있지만 ‘최신’이라고 표시하지 않습니다. 재시도가 성공하면 경고를 지우고 조회한 목록을 사용합니다.
...
검색·버전 조회는 요청 순번과 검색 환경으로 최신 응답만 반영합니다. 새 입력, 초기화, 패키지 타입/채널/인덱스/배포판 변경, 화면 종료 이후 도착한 응답은 무시합니다. Enter·재시도는 대기 중인 자동 검색을 취소하고 한 번만 실행합니다.
