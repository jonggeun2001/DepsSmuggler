...
| 충돌 버전과 탐색 한도 | 현재 공통 BFS는 최선 후보와 다운로드 대상으로 유지한 충돌 버전의 전이 의존성을 함께 탐색합니다. 이름·버전·아키텍처로 중복 큐잉을 막고, 단일 루트에서 고유 패키지 10,000개를 처리한 뒤 남은 작업이 있으면 오류를 반환합니다. 아래 초안처럼 모든 후보를 무조건 부모 엣지로 연결하지는 않습니다. |
