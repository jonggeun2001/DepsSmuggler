...
| 설계 주제 | 현재 구현·차이 |
|-----------|----------------|
| `src/core/downloaders/os/`와 통합 `OSPackageDownloader` | 실제로는 `src/core/downloaders/yum/`, `apt/`, `apk/`의 개별 다운로더와 `os-shared/base-downloader.ts`를 사용합니다. 초안의 통합 클래스는 없습니다. |
...
| 메타데이터·의존성 해결 | 각 관리자 디렉터리의 `metadata-parser.ts`, `dependency-resolver.ts`와 `os-shared/base-resolver.ts`, `dependency-tree.ts`. |
