...
- 다운로드 성공 시 설치 스크립트를 먼저 생성한 뒤, 완료된 파일을 `packages/` 아래에 배치한 `packages-<timestamp>.zip` 또는 `.tar.gz`를 만듭니다. 아카이브 최상위에는 `install.sh`와 `install.ps1`을 포함하고, 원본 출력 디렉터리에도 두 스크립트를 유지합니다. 완료 목록에 없는 이전 파일은 아카이브에 추가하지 않습니다. 스크립트 생성에 실패하면 이번 실행의 아카이브를 만들지 않고 종료 코드 `1`을 반환합니다.
...
- Maven은 버전 충돌로 JAR 선택에서 제외된 버전도 의존성 분석에 필요한 POM을 별도 다운로드 항목으로 포함합니다. 예를 들어 Flink 1.20.5에서 Kryo 2.24.0을 선택해도 `chill-java:0.7.6`이 참조하는 `kryo:2.21`의 POM과 필요한 하위·부모·BOM POM을 함께 전달합니다. 다운로드 목록에는 이 POM-only 항목도 포함되므로 표시 개수는 JAR 개수와 다를 수 있습니다. 제외된 버전의 JAR를 추가로 선택하는 동작은 아닙니다.
...
- YUM 패키지의 버전과 RPM release는 `1.0`, `01` 같은 원래 표기를 유지합니다. 이전 캐시에 숫자로 변환된 버전 정보가 있으면 저장소에서 다시 읽습니다.
