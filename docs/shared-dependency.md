...
interface DependencyResolverOptions {
  ...
  // CLI download 기본값. PipResolver를 직접 호출하면 기본값은 10입니다.
  resolveRootArtifactsOnly?: boolean; // --no-deps: 루트만 조용히 처리
...
`resolveAllDependencies`의 `maxDepth` 기본값은 `5`이며 CLI `download --deps`가 사용하는 값입니다. 각 리졸버는 이 깊이까지 해결된 패키지를 결과에 포함합니다. pip는 경계 노드에 적용 가능한 의존성이 남아 있으면 하위 확장을 생략하고 깊이 정보를 담은 경고를 남기지만, 직접 루트를 실패로 처리하지 않습니다. 반면 `resolveRootArtifactsOnly`(`--no-deps`)는 pip/Conda 루트 아티팩트만 조회하는 의도적인 root-only 모드로, 깊이 제한 경고 없이 조용히 종료합니다. pip `PipResolver.resolveDependencies`를 직접 호출할 때의 `maxDepth` 기본값은 `10`입니다.
