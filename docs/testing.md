...
`m2repo.example` 그룹 단독 및 `example` 그룹과 함께 있는 CLI·GUI 경로도 실제 스크립트로 검증하여 그룹 이름을 GUI 저장소 폴더로 오인하거나 다른 그룹의 파일을 설치하는 회귀를 방지합니다. 모호하거나 두 구조에 분산된 원본은 설치 전에 실패하는지도 확인합니다.
