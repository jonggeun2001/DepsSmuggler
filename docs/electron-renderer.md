...
- `UpdateNotification.tsx`가 `updater:status` 이벤트를 구독하고 초기 상태를 조회합니다. 구독 전에 새 버전 발견·다운로드 시작·설치 준비가 완료되어도 초기 상태로 모달을 복원합니다. 늦게 도착한 초기 응답은 최신 이벤트나 사용자의 모달 닫기를 덮어쓰지 않습니다.
