...
#### 주요 기능
- **조건부 2단계 다운로드 프로세스**: 설정에서 의존성 자동 포함이 켜져 있으면 의존성 확인 → 다운로드 시작
- 의존성 자동 포함이 켜진 경우에만 의존성 확인 단계에서 전이 의존성을 해결
- 의존성 자동 포함이 꺼져 있으면 장바구니 원본 패키지로 바로 다운로드 시작
- **의존성 해결 진행 상황 표시**: 각 패키지별 실시간 진행 로그
- 의존성 포함 다운로드 시 Collapse 트리 구조로 패키지별 의존성 목록 표시
...
설정에서 의존성 자동 포함이 켜져 있으면 다운로드 전에 의존성을 먼저 확인하여 전체 패키지 목록을 보여줍니다. 설정이 꺼져 있으면 이 단계를 생략하고 바로 다운로드할 수 있습니다.
...
1. 원본 장바구니 항목을 그대로 다운로드 목록으로 사용
2. `depsResolved`를 자동으로 `true`로 전환
3. 의존성 확인 버튼 없이 즉시 다운로드 시작 가능
