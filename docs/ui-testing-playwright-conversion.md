...
| `UI-DL-004` | `tests/e2e/download-cancel-retry.spec.ts` | 느린 다운로드 취소 유지, 취소 후 늦은 완료 이벤트 무시, 실패 후 개별 재시도, 전달 실패가 남은 취소 화면에서의 새 세션 재시도, runtime download/cancel 시도 횟수 |
