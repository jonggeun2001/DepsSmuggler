...
- 모든 조회/변경은 메인 프로세스의 파일별 큐에서 순서대로 실행합니다. 동시 추가는 앞선 저장 결과를 읽어 변경하고, 실패한 요청 뒤에도 다음 요청을 처리합니다. 전체 저장/전체 삭제는 그 순서에서 명시적으로 목록을 대체합니다.
- 저장은 임시 파일 작성 후 원자적 rename으로 교체합니다. `history:load`는 잘못된 JSON·배열/항목 형태·I/O 실패를 기록하고 빈 배열을 반환하되 기존 파일을 보존합니다. 항목은 비어 있지 않은 문자열 ID를 가진 객체여야 하며, 기존 추가 데이터 필드는 유지합니다.
- 추가/삭제는 손상된 파일을 빈 배열로 간주하여 덮어쓰지 않고 실패합니다. 저장/추가/삭제/전체 삭제의 검증·I/O 실패는 호출자에게 전파하며, 명시적인 전체 저장/전체 삭제로 복구할 수 있습니다. [원자적 저장의 범위](shared-file-path.md#설정히스토리-json-저장)를 참고하세요.
