...
Maven의 충돌로 제외된 버전에서 수집한 descriptor POM도 독립적인 POM-only 다운로드 항목으로 전달됩니다. 압축기와 설치 스크립트는 이 항목의 원본 POM과 체크섬을 같은 GAV 경로에 보존합니다. 최종 JAR 선택 목록만 다시 구성하거나 내부 캐시 파일을 임의로 복사해 반출 목록을 만들지 않습니다.
