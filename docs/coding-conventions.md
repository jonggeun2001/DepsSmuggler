...
테스트 대상은 운영 코드에서 가져와 호출합니다. 기대값을 만들기 위한 fixture나 외부 시스템 mock은 사용할 수 있지만, 운영 로직을 테스트 파일 안에 다시 구현한 뒤 그 복사본만 검증하지 않습니다. 예를 들어 파일 분할 규칙은 별도의 `generatePartName` 복사본이 아니라 `FileSplitter`가 실제로 만든 파일로 확인합니다.

중복 여부는 테스트 이름이나 본문 모양만으로 판단하지 않습니다. 대상 구현, 입력 조건, 관찰 결과, 실행 경계가 같은지 비교하고, 삭제할 검증을 남은 테스트가 담당하는지 확인합니다. 단위 테스트와 실제 파일·HTTP·IPC 조합 테스트, 플랫폼별 조건부 테스트는 같은 기능을 다루더라도 보호하는 경계가 다르면 유지합니다.
