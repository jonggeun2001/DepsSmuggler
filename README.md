...
- **데스크톱 업데이트** — 배포 앱에서 업데이트 확인, 서식 있는 패치 노트, 다운로드, 설치 UI를 제공합니다. 설정 토글의 연결 범위는 [현재 동작 범위](#현재-동작-범위)를 참고하세요.
...
검색 요청이 실패하면 검색창 아래에 원인과 **다시 시도**를 표시합니다. 인증서 오류는 **CA 설정 열기**로 이동할 수 있고, 버전 조회 실패 시 대체 버전이라는 안내를 표시합니다. [검색 오류와 재시도](docs/search-errors.md)를 참고하세요.
