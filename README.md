...
| 설치 스크립트 범위 | npm은 전달된 `.tgz`를 오프라인으로 설치해 스크립트 폴더의 `node_modules`에 배치합니다. Conda 항목은 아직 pip 명령으로 처리하므로 Conda 오프라인 설치를 보장하지 않습니다. 생성기별 범위는 [Packagers](docs/packagers.md)를 참고하세요. |
