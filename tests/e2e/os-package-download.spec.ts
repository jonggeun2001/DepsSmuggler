...
test('OS 패키지 검색부터 전용 다운로드 결과 화면까지 연결된다', async ({ page }) => {
  ...
  await page.getByText('OS 패키지', { exact: true }).click();
  await page.getByRole('button', { name: '다음' }).click();
  await page.getByText('YUM', { exact: true }).click();
  await page.getByRole('button', { name: '다음' }).click();
  ...
  await page.getByRole('textbox', { name: '다운로드 폴더 경로' }).fill('/tmp/depssmuggler-e2e');
  await page.getByRole('button', { name: '출력 옵션 ▼' }).click();
  await page.locator('.output-type-card .card-label', { hasText: /^로컬 저장소$/ }).click();
  await page.getByRole('button', { name: '다운로드 시작' }).click();

  await expect(page.getByText('다운로드 완료!')).toBeVisible();
  await expect(page.locator('.summary-value', { hasText: /^로컬 저장소$/ })).toBeVisible();
});
