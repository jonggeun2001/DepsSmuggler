...
import { sanitizeCacheKey } from '../filename-utils';
import { calculateFileChecksum, verifyFileChecksum } from '../integrity/checksum';
...
export interface ArtifactCacheManifest {
  version: string;
...
interface ArtifactCacheManifestJson {
  version: string;
  entries: Array<[string, ArtifactCacheEntry]>;
  totalSize: number;
  lastUpdated: string;
}
...
export class ArtifactCacheManager {
  ...
  private manifest: ArtifactCacheManifest | null = null;
   ...
   * 캐시 목록 조회
   */
  async getCacheEntries(): Promise<ArtifactCacheEntry[]> {
    await this.ensureManifest();
  ...
  private async loadManifest(): Promise<void> {
    ...
    if (await fs.pathExists(manifestPath)) {
      try {
        const data: ArtifactCacheManifestJson = await fs.readJson(manifestPath);
        this.manifest = {
  ...
  private async saveManifest(): Promise<void> {
    ...
    const data: ArtifactCacheManifestJson = {
   ...
   * 빈 매니페스트 생성
   */
  private createEmptyManifest(): ArtifactCacheManifest {
    return {
      version: CACHE_VERSION,
      entries: new Map(),
      totalSize: 0,
...
// 싱글톤 인스턴스
let cacheManagerInstance: ArtifactCacheManager | null = null;

export function getCacheManager(options?: CacheOptions): ArtifactCacheManager {
  if (!cacheManagerInstance) {
    cacheManagerInstance = new ArtifactCacheManager(options);
  }
  return cacheManagerInstance;
}

export async function initializeCacheManager(options?: CacheOptions): Promise<ArtifactCacheManager> {
  const manager = getCacheManager(options);
  await manager.initialize();
  return manager;
}

export type CacheEntry = ArtifactCacheEntry;
export type CacheManifest = ArtifactCacheManifest;
export { ArtifactCacheManager as CacheManager };
