 ...
 * 캐시 결과
 */
export interface CacheStoreGetResult<T> {
  /** 데이터 */
  data: T;
  /** 캐시에서 가져왔는지 */
  fromCache: boolean;
...
export class CacheStore<T> {
  ...
  /** 메모리 캐시 */
  private cache: Map<string, CacheStoreEntry<T>> = new Map();
  ...
  getFromDisk(key: string): T | undefined {
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        const entry = JSON.parse(content) as CacheStoreEntry<string>;
   ...
   * 디스크에 저장
   */
  private writeToDisk(key: string, entry: CacheStoreEntry<T>): void {
    ...
    try {
      ...
      const diskEntry: CacheStoreEntry<string> = {
   ...
   * 메모리 캐시 항목 유효성 검사
   */
  private isValid(entry: CacheStoreEntry<T>): boolean {
   ...
   * 디스크 캐시 항목 유효성 검사
   */
  private isDiskEntryValid(entry: CacheStoreEntry<string>): boolean {
...
export type CacheEntry<T> = CacheStoreEntry<T>;
export type CacheManagerOptions<T> = CacheStoreOptions<T>;
export type CacheManagerStats = CacheStoreStats;
export type CacheGetResult<T> = CacheStoreGetResult<T>;
export { CacheStore as CacheManager };
