import type { InstallationCandidate } from './pip-candidate';
...
function createCandidates(): InstallationCandidate[] {
  return Array.from({ length: 40 }, (_, index) => {
    ...
    return {
      name: 'sample',
      version: String(1 + (index % 4)),
      filename,
      url: filename,
      packageType: 'wheel',
      wheelInfo: parseWheelFilename(filename)!,
      hash: index % 5 ? 'other' : 'allowed',
      isYanked: index % 7 === 0,
    };
  });
}
...
describe('CandidateEvaluator sort cost', () => {
  it.each([true, false])(
    'binary 선호=%s에서 기존 비교 순서와 후보 객체를 유지한다',
    (preferBinary) => {
      ...
      candidates.push({
        name: 'sample',
        version: '5',
        filename: 'sample-5.tar.gz',
        url: 'sdist',
        packageType: 'sdist',
      });
      const snapshot = [...candidates];
      const expected = candidates
        .filter((candidate) => evaluator.isApplicable(candidate))
        .sort((a, b) =>
