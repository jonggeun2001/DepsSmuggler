...
/**
 * Maven 중복 제거 인덱스 - 버전 충돌 및 중복 감지
 */
export class MavenDedupeIndex {
  /** G:A별 해결된 버전 */
...
export class DependencyResolutionSkipper {
  ...
  private dedupeIndex: MavenDedupeIndex;
  ...
  constructor(
    ...
    dedupeIndex?: MavenDedupeIndex
  ) {
    this.coordinateManager = coordinateManager || new CoordinateManager();
    this.dedupeIndex = dedupeIndex || new MavenDedupeIndex();
  }
  ...
  skipResolution(
    ...
    if (this.dedupeIndex.isVersionConflict(coordinate)) {
    ...
    if (this.dedupeIndex.isDuplicate(coordinate)) {
   ...
   */
  recordResolved(coordinate: MavenCoordinate): void {
    this.dedupeIndex.recordResolved(coordinate);
  }
   ...
   */
  getResolvedVersion(groupId: string, artifactId: string): string | undefined {
    return this.dedupeIndex.getResolvedVersion(groupId, artifactId);
  }
  ...
  getDedupeIndex(): MavenDedupeIndex {
    return this.dedupeIndex;
  }

  getCacheManager(): MavenDedupeIndex {
    return this.dedupeIndex;
  }
...
export { MavenDedupeIndex as CacheManager };
